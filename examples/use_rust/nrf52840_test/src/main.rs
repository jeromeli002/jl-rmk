...
async fn main(spawner: Spawner) {
    ...
    // Initialize IO Pins
    let (row_pins, col_pins) = config_matrix_pins_nrf!(peripherals: p, input: [P0_04, P1_09], output:  [P0_15, P0_17]);

    // Initialize the ADC.
    // We are only using one channel for detecting battery level
    let adc_pin = p.P0_29.degrade_saadc();
    let is_charging_pin = Input::new(p.P0_02, embassy_nrf::gpio::Pull::Up);
    ...
    let pin_a = Input::new(p.P1_13, embassy_nrf::gpio::Pull::None);
    let pin_b = Input::new(p.P0_28, embassy_nrf::gpio::Pull::None);
